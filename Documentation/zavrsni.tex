...
Drugi pristup treniranju neuronske mreže je korištenje algoritma backpropagation.
Za razliku od genetskog algoritma koji nema nikakvo znanje o tome u kojem smjeru se nalazi optimalno rješenje, ovaj algoritam računa odstupanja dobivenog rezultata od očekivanog za svaki neuron te primjenom gradijentnog spusta.

\subsubsection{Ukupna greška}
Ukupna greška neuronske mreže definirana je kao suma kvadratnih odstupanja, odnosno kao:
\[
    E_{total} = \sum\frac{1}{2}(y_i-\hat{y})^2
\]
gdje je $y_i$ očekivana vrijednost izlaza za određeni neuron izlaznog sloja, a $\hat{y}$ stvarna vrijednost izlaza.

\subsubsection{Promjena težina izlaznog sloja}
Pogreška $\delta_k$ za svaki neuron izlaznog sloja računa se kao:
\[
    \delta_k = o_k(1-o_k)(t_k-o_k)
\]
gdje je $o_k$ izlaz promatranog neurona, a $t_k$ očekivani izlaz za taj neuron.

\subsubsection{Promjena težina skrivenog sloja}
Pogreška $\delta_h$ za svaki neuron skrivenog sloja računa se kao:
\[
    \delta_h = o_h(1-o_h) \displaystyle\sum_s w_{hs}\delta_s
\]
gdje je $o_h$ izlaz promatranog neurona, a suma se računa za svaki izlaz $s$ iz neurona.
